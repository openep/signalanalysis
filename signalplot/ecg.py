...
def plot_ecg(ecgs: Union[List["Ecg"], "Ecg"],
    ...
    Parameters
    ----------
    ecgs : signalanalysis.ecg.Ecg or list of signalanalysis.ecg.Ecg
        ECG data class
    legend_ecg : list of str, optional
        List of names for each given set of ECG data e.g. ['BCL=300ms', 'BCL=600ms']. Must be the same length as the
        list of ECG data, default=None
    ...
    for (ecg, label, colour, linestyle, linewidth) in zip(ecgs, legend_ecg, colours_ecg, linestyles_ecg,
        ...
        for key in plot_sequence:
            ax[key].plot(ecg.data.index, ecg.data[key])
...
def plot_dataframe(ecgs: Union[List[pd.DataFrame], pd.DataFrame],
                   legend_ecg: Optional[List[str]] = None,
                   linewidths_ecg: float = 2,
                   limits: Union[list, float, None] = None,
                   legend_limits: Optional[List[str]] = None,
...
def __init_axes(plot_sequence: List[str],
    ...
    if single_fig:
        fig = plt.figure()
        for i_key, key in enumerate(plot_sequence):
            ax[key] = fig.add_subplot(2, 6, i_key+1)
    else:
        fig = dict()
        for key in plot_sequence:
            fig[key] = plt.figure()
